...
RoutingTableEntry::RoutingTableEntry (Ptr<NetDevice> dev, Ipv4Address dst, bool vSeqNo, uint32_t seqNo,
                                      Ipv4InterfaceAddress iface, uint16_t hops, Ipv4Address nextHop, Ipv4Address secondHop,
                                      Time lifetime)
...
bool
RoutingTableEntry::InsertPrecursor (Ipv4Address id) //プリカーサ・リストにまだプリカーサが存在しない場合、プリカーサ・リストにプリカーサを挿入する。
{
...
bool
RoutingTableEntry::LookupPrecursor (Ipv4Address id) //アドレスで前駆体を検索
{
  NS_LOG_FUNCTION (this << id);
...
bool
RoutingTableEntry::DeletePrecursor (Ipv4Address id) //Delete precursor
{
  NS_LOG_FUNCTION (this << id);
...
RoutingTable::AddRoute (RoutingTableEntry & rt)
{
  ...
  if (rt.GetFlag () != IN_SEARCH)
    ...
    //std::pair ２つの異なる方の値を保持する組を表現するためのクラス
  std::pair<std::map<Ipv4Address, RoutingTableEntry>::iterator, bool> result =
    m_ipv4AddressEntry.insert (std::make_pair (rt.GetDestination (), rt)); //(std::map)m_ipv4AddressEntry.insert:平衡２分木二要素を挿入
