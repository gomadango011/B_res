...
private:
  ...
  uint16_t m_WHCheckRateLimit;           ///< Maximum number of WHCS per second.
  ...
  /// IP protocol
  Ptr<Ipv4> m_ipv4;
  /// 各IPインターフェイスごとに生のユニキャストソケット、マップソケット -> ifaceアドレス(IP + マスク)
  ...
  uint32_t m_seqNo;
  ///WHCheck sequence number
  uint32_t m_WHseqNo;
  /// Handle duplicated RREQ
  IdCache m_rreqIdCache;
  /// Handle duplicated WHCS
  IdCache m_WHCheckIdCache;
  /// Handle duplicated WHCS
  IdCache m_WHEndIdCache;
 ...
 /// WHCSレート制御に使用されるWHCS数
  uint16_t m_WHCheckCount;
...
private:
  ...
  /**
   * 送信元ノードが1つの宛先に対して繰り返し経路探索を試みる場合、拡大リング探索技法が使用される。
   * \param dst the destination IP address
   */
  void ScheduleRreqWHCheck (Ipv4Address dst);
  ...
  /// Receive WHCS
  void RecvWHCheck (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src);
  /// Receive WHCE
  void RecvWHCheckEnd (Ptr<Packet> p, Ipv4Address my,Ipv4Address src);
  //\}
  ...
  /// Send WHCR(WH check Request)
  void SendWHCheck (RrepHeader rrepHeader);

  /// Send RREP
  void SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin);
  
  /// Send RREP
  void SendWHCheckEnd (WHCheckHeader const & WHCheckHeader, RoutingTableEntry const & toOrigin, Ipv4Address receiver);
 ...
 /// RREQ rate limit timer
  Timer m_WHCheckRateLimitTimer;
  /// WHCSカウントをリセットし、WHCSレート制限タイマーを遅延1秒でスケジュールする。
  void WHCheckRateLimitTimerExpire ();

  /// Map IP address + RREQ timer.
  std::map<Ipv4Address, Timer> m_addressReqTimer;
  std::map<Ipv4Address, Timer> m_addressWHCTimer;
  /**
