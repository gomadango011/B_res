...
int main (int argc, char **argv)
{
  ...
  if(!p_size.is_open())
  {
    ...
    p_size << "検知率: " << WHDM_num / WHDC_num << std::endl;
...
//   malicious.Add(nodes.Get(2));//WH2
  ...
  AnimationInterface::SetConstantPosition (nodes.Get (size-1), network_size, 0);

  //WHノードを配置
  AnimationInterface::SetConstantPosition (nodes.Get (1), 100, 0); //WH1
  AnimationInterface::SetConstantPosition (nodes.Get (2), 350, 0); //WH2

  //周辺ノードを配置
  AnimationInterface::SetConstantPosition (nodes.Get (3), 430, 0);
